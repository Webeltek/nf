...
export class DropinComponent implements OnInit {
  ...
  private _dropinEvents : CalendarEvent[]=[];
  ...
  public set events(events : CalendarEvent[]){
    this._events = events;
    this.dropinEvents= events;
  }
  public get dropinEvents(){
    return this._dropinEvents;
  }

  public set dropinEvents(events: CalendarEvent[]){
    this._dropinEvents= events.filter(evt => {
      //console.log("dropin evt.bookname",evt.bookname);
  ...
  externalDrop(events: CalendarEvent[]) {
    if (events.length>0){
      for (let event of events){
        if (this.externalDropinEvents.indexOf(event) === -1) {
  ...
  eventTimesChanged({
    ...
    const externalIndex = this.externalDropinEvents.indexOf(event);
    ...
    if (externalIndex > -1) { // if event is dropped from chips to calendar
      ...
      this.authService.dbUpdateVippsPayment(
        this.tokenStorage.getUser().id,
        this.externalDropinEvents[externalIndex].paymntref,
        true)
      .subscribe((resp)=>{
        console.log("DA bookname",this.externalDropinEvents[externalIndex].bookname);
        this.httpService.generatePythEvent({
          user_id :this.tokenStorage.getUser().id,
          title: this.tokenStorage.getEventTitle(
            this.externalDropinEvents[externalIndex].bookname,event.userId,this.users),
          paymntref : event.paymntref,
          bookname : this.externalDropinEvents[externalIndex].bookname,
          roomname : "",
    ...
    if (resp && resp!=="access token expired"){
      ...
      for (let paymnt of paymnts ){
        if(paymnt.is_consumed===false && paymnt.bookname==="Drop-in"){
          ...
          this.externalDropinEvents.push(extEvent);
        }
    ...
    this.httpService.getUsers().subscribe(
      {
          next: (response) => {
            if(response.hasOwnProperty('users') && response.hasOwnProperty('events') 
              ...
              const dropinTitles = bookTitles.filter((title)=>title==='Drop-in');  // single element array title Drop-in
              //console.log("DemoApp  roomNamesArr$.subscribe typeof roomNamesArr:", roomNamesArr);
              this.books = [...dropinTitles];
